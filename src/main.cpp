...
#include "json/adaptors/person.hpp"
...
int main(int argc, char *argv[])
{
    ...
    std::shared_ptr<com::example::database> db(new com::example::database("db.sqlite"));

    auto arthur = person("Arthur", "Farias", 31);
    ...
    router.get(R"(^/persons$)", [&](beast_http_request r, http_context c)
               {
        // Send content message to client and wait to receive next request
nlohmann::json j;
            {
            odb::transaction t(db->begin());
            auto q = odb::query<person>(odb::query<person>::first == "Arthur");
            ...
            // std::vector<person> output;
                ...
                if (!r.empty())
                {
                    for (auto &p : r)
                    {
                        printf("%s %s: %d\n", p.first().c_str(), p.last().c_str(), p.age());
                        j.emplace_back(p);
                        db->erase(p);
                    }
                }
            ...
            std::string s = j.dump();
        c.send(make_200<beast::http::string_body>(r, s, "application/json")); });
